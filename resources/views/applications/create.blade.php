<x-app-layout>
  ...
  {{-- Outstanding shortfall / credit from a prior loan — informational,
       never blocks submission. Disclosed here so the client sees it before
       committing to a new agreement, not just after on the paperwork. --}}
  ...
  <div class="kc-alert-warning mb-6 flex items-start gap-2">
    ...
    <div>
      <p class="font-semibold">Outstanding balance from a previous loan: R{{ number_format($outstandingShortfall, 2) }}</p>
      <p class="text-xs mt-0.5">This will be carried forward and disclosed as a separate line item on your new loan agreement — it will not be added to your new loan's principal or instalments.</p>
    </div>
  </div>
  @endif

  @if(($outstandingCredit ?? 0) > 0)
  <div class="kc-alert-success mb-6 flex items-start gap-2">
    <svg class="w-4 h-4 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
      <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
    </svg>
    <div>
      <p class="font-semibold">Credit balance: R{{ number_format($outstandingCredit, 2) }}</p>
      <p class="text-xs mt-0.5">From a previous overpayment — this will be applied to reduce your next instalment.</p>
    </div>
  </div>
  @endif
