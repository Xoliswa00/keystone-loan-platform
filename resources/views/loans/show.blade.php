    ...
    <div class="lg:col-span-2 space-y-5">
      ...
      <div class="kc-card">
        <div class="flex items-center justify-between mb-4">
          ...
          <span class="text-xs text-kc-charcoal/70">Total due: <strong>R {{ number_format($loanApplication->loanfee->total_due, 2) }}</strong></span>
          @endif
        </div>
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
          <div><p class="text-xs text-kc-charcoal/70">Amount</p><p class="font-display text-lg font-bold text-kc-navy">R {{ number_format($loanApplication->loan_amount, 2) }}</p></div>
          <div><p class="text-xs text-kc-charcoal/70">Term</p><p class="font-semibold">{{ $loanApplication->loan_term_months ?? 1 }} month(s)</p></div>
          <div><p class="text-xs text-kc-charcoal/70">Applied</p><p class="font-semibold text-xs">{{ $loanApplication->created_at->format('d M Y') }}</p></div>
          <div><p class="text-xs text-kc-charcoal/70">Purpose</p><p class="font-semibold text-xs truncate">{{ $loanApplication->purpose ?? '—' }}</p></div>
        </div>
      </div>
      ...
      <div class="kc-card border-2 border-kc-gold/40">
        ...
        <div class="grid grid-cols-3 gap-3 text-center mb-4">
          <div><p class="text-xs text-kc-charcoal/70">Amount</p><p class="font-display text-lg font-bold text-kc-navy">R {{ number_format($pendingCounterOffer->amount, 2) }}</p></div>
          <div><p class="text-xs text-kc-charcoal/70">Term</p><p class="font-semibold">{{ $pendingCounterOffer->months }} month{{ $pendingCounterOffer->months > 1 ? 's' : '' }}</p></div>
          <div><p class="text-xs text-kc-charcoal/70">Instalment</p><p class="font-semibold">R {{ number_format($pendingCounterOffer->instalment, 2) }}/mo</p></div>
        </div>
        <p class="text-xs text-kc-charcoal/70 mb-3">Product: {{ $pendingCounterOffer->product->name }}</p>
      ...
      <div class="kc-card">
        ...
        <table class="kc-table">
          <tbody>
            <tr><td class="text-kc-charcoal/70 text-xs">Principal</td><td class="font-semibold">R {{ number_format($loanApplication->loan_amount, 2) }}</td></tr>
            <tr><td class="text-kc-charcoal/70 text-xs">Initiation Fee (incl. VAT)</td><td>R {{ number_format($loanApplication->loanfee->initiation_fee, 2) }}</td></tr>
            <tr><td class="text-kc-charcoal/70 text-xs">Service Fee</td><td>R {{ number_format($loanApplication->loanfee->service_fee, 2) }}</td></tr>
            <tr><td class="text-kc-charcoal/70 text-xs">Interest</td><td>R {{ number_format($loanApplication->loanfee->interest_amount, 2) }}</td></tr>
    ...
    <div class="space-y-4">
      <div class="kc-card">
        ...
        <div class="space-y-2">
          ...
          <div class="pt-2 border-t border-kc-silver-light">
            <p class="text-xs text-kc-charcoal/70 mb-2">Want to pay off early?</p>
          ...
          <div class="pt-2 border-t border-kc-silver-light">
            <p class="text-xs text-kc-charcoal/70 mb-2">NCA Documents</p>
      ...
      <div class="kc-card">
        ...
        <table class="w-full text-xs">
          <tr class="border-b border-kc-silver-light/60"><td class="py-1.5 text-kc-charcoal/70">Remaining</td><td class="font-bold text-kc-navy">R {{ number_format($loanApplication->loan->remaining_balance ?? 0, 2) }}</td></tr>
          <tr class="border-b border-kc-silver-light/60"><td class="py-1.5 text-kc-charcoal/70">Disbursed</td><td>{{ $loanApplication->loan->disbursed_date ? \Carbon\Carbon::parse($loanApplication->loan->disbursed_date)->format('d M Y') : '—' }}</td></tr>
          <tr><td class="py-1.5 text-kc-charcoal/70">Next payment</td><td class="{{ $loanApplication->loan->next_payment_date && \Carbon\Carbon::parse($loanApplication->loan->next_payment_date)->isPast() ? 'text-red-600 font-semibold' : '' }}">{{ $loanApplication->loan->next_payment_date ? \Carbon\Carbon::parse($loanApplication->loan->next_payment_date)->format('d M Y') : '—' }}</td></tr>
        </table>
      </div>
      @endif
    </div>
