  ...
  <div class="flex justify-end items-center gap-2 mb-3">
    ...
    <div x-data="{open:false}">
      ...
      <div x-show="open" x-transition class="absolute mt-2 z-10 bg-white border border-kc-silver-light rounded-lg p-3 shadow-lg w-80">
        <p class="text-xs text-kc-charcoal/70 mb-2">
    ...
    <div class="kc-card mb-6">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-3">
        <div class="kc-stat-card text-center">
          <p class="text-xs text-kc-charcoal/70 uppercase tracking-wider">Risk Score</p>
          <p class="font-display text-xl font-bold text-kc-navy mt-1">{{ $cloDecision->risk_score }}/100</p>
        </div>
        <div class="kc-stat-card text-center">
          <p class="text-xs text-kc-charcoal/70 uppercase tracking-wider">Compliance</p>
      ...
      @if(!empty($cloDecision->fraud_flags))
        <p class="text-xs text-kc-charcoal/70 uppercase tracking-wider mb-1">Flags</p>
      ...
      @if(!empty($cloDecision->required_actions))
        <p class="text-xs text-kc-charcoal/70 uppercase tracking-wider mb-1">Required Actions</p>
      ...
      @endif
      <p class="text-xs text-kc-charcoal/70 mt-3">Evaluated {{ $cloDecision->evaluated_at->format('d M Y H:i') }} · advisory only, does not block manual approval/rejection</p>
    </div>
  @endif
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    <div class="lg:col-span-2 space-y-5">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div class="kc-stat-card text-center">
          <p class="text-xs text-kc-charcoal/70 uppercase tracking-wider">Requested</p>
          <p class="font-display text-xl font-bold text-kc-navy mt-1">R {{ number_format($application->loan_amount,2) }}</p>
        </div>
        <div class="kc-stat-card text-center">
          <p class="text-xs text-kc-charcoal/70 uppercase tracking-wider">Total Due</p>
        ...
        <div class="kc-stat-card text-center">
          <p class="text-xs text-kc-charcoal/70 uppercase tracking-wider">Rejections</p>
          ...
          <table class="w-full text-sm">
            <tbody>
              ...
              <tr class="border-b border-kc-silver-light/60">
                <td class="py-2 text-xs text-kc-charcoal/70 pr-3">{{ $k }}</td>
      ...
      <div class="kc-card">
        ...
        <div class="mt-4 pt-4 border-t border-kc-silver-light grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
          <div><p class="font-display text-lg font-bold {{ ($profile->avg_days_to_zero ?? 99) < 7 ? 'text-red-600' : 'text-kc-navy' }}">{{ $profile->avg_days_to_zero ?? '—' }}</p><p class="text-[10px] text-kc-charcoal/70">Days to R500 after payday</p></div>
          <div><p class="font-display text-lg font-bold text-kc-navy">R {{ number_format($profile->verified_income_amount ?? 0, 0) }}</p><p class="text-[10px] text-kc-charcoal/70">Verified salary</p></div>
          <div><p class="font-display text-lg font-bold text-kc-navy">R {{ number_format($profile->detected_regular_debits ?? 0, 0) }}</p><p class="text-[10px] text-kc-charcoal/70">Detected debits</p></div>
          <div>
            ...
            <p class="text-[10px] text-kc-charcoal/70 mt-1">Bank risk</p>
          </div>
        </div>
        @if($profile->bank_statement_risk_reason)
          <p class="text-xs text-kc-charcoal/70 mt-2">{{ $profile->bank_statement_risk_reason }}</p>
        @endif
        @endif
      </div>
      @endif
          ...
          <table class="kc-table">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td data-label="Date" class="text-xs text-kc-charcoal/70">{{ $prev->created_at->format('d M Y') }}</td>
              </tr>
              @endforeach
            </tbody>
          </table>
      ...
      <div class="kc-card border-amber-300/50">
          ...
          <table class="kc-table">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td data-label="Age" class="text-xs text-kc-charcoal/70">{{ $adj->created_at->diffForHumans(null, true) }}</td>
              </tr>
              @endforeach
            </tbody>
          </table>
        ...
        <p class="text-xs text-kc-charcoal/70 mt-3">
      ...
      <div class="kc-card">
        ...
        <div class="flex gap-2 mb-3">
          ...
          <div>
            <p class="text-xs text-kc-charcoal/70">{{ $note->admin_name }} · {{ \Carbon\Carbon::parse($note->created_at)->diffForHumans() }}</p>
    ...
    <div class="space-y-5">
      ...
      <div class="kc-card" x-data="{
        ...
        <p class="text-xs text-kc-charcoal/70 mb-3">This client doesn't currently qualify for any amount on any available product — a counter-offer isn't possible.</p>
        @else
      ...
      <div class="kc-card">
        ...
        <p class="text-xs font-semibold text-kc-charcoal/70 mt-3 mb-1 uppercase">KYC</p>
      ...
      <div class="kc-card">
        ...
        <div class="flex items-center justify-between py-1.5 border-b border-kc-silver-light/60 last:border-0">
          <div>
            ...
            <p class="text-[10px] text-kc-charcoal/70">{{ $ag->reference }}</p>
          </div>
          <div class="flex items-center gap-1.5">
