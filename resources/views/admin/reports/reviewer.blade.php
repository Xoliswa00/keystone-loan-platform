  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    <div class="lg:col-span-2 space-y-5">
      ...
      <div class="kc-card">
        ...
        <p class="text-xs font-semibold text-kc-charcoal/70 uppercase tracking-wider mb-2 mt-3">{{ ucfirst($status ?: 'Unknown') }} ({{ $group->count() }})</p>
        <div class="kc-table-scroll">
          <table class="kc-table">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td data-label="Date" class="text-xs text-kc-charcoal/70">{{ \Carbon\Carbon::parse($row->created_at)->format('d M Y') }}</td>
              </tr>
              @endforeach
            </tbody>
          </table>
    ...
    <div class="kc-card">
      ...
      <div class="p-3 rounded-lg border border-red-200 bg-red-50 mb-2">
        ...
        <p class="text-xs text-kc-charcoal/70">{{ $d->customer_code??'—' }}</p>
        <p class="text-xs font-semibold text-red-600 mt-1">R {{ number_format($d->remaining_balance??0,2) }}</p>
      </div>
      @empty
      <p class="text-sm text-kc-charcoal/70 text-center py-4">No defaults.</p>
      @endforelse
    </div>
  </div>
</x-app-layout>
