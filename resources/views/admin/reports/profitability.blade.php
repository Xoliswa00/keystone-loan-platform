<x-app-layout>
  ...
  <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    <div class="kc-stat-card">
      <span class="text-xs font-semibold uppercase tracking-wider text-kc-charcoal/70">Principal Deployed</span>
      <p class="font-display text-2xl font-semibold text-kc-navy mt-2">R {{ number_format($totalPrincipalDisbursed, 2) }}</p>
    </div>
    <div class="kc-stat-card">
      <span class="text-xs font-semibold uppercase tracking-wider text-kc-charcoal/70">Gross Income</span>
      <p class="font-display text-2xl font-semibold text-kc-navy mt-2">R {{ number_format($totalGrossIncome, 2) }}</p>
      <p class="text-xs text-kc-charcoal/70 mt-1">Interest + Fees recognised (GL), excl. VAT</p>
    </div>
    <div class="kc-stat-card">
      <span class="text-xs font-semibold uppercase tracking-wider text-kc-charcoal/70">Collected</span>
      <p class="font-display text-2xl font-semibold text-emerald-600 mt-2">R {{ number_format($totalCollected, 2) }}</p>
      <p class="text-xs text-kc-charcoal/70 mt-1">Cash received in period</p>
    </div>
    <div class="kc-stat-card">
      <span class="text-xs font-semibold uppercase tracking-wider text-kc-charcoal/70">Net Revenue</span>
      <p class="font-display text-2xl font-semibold {{ $netRevenue >= 0 ? 'text-kc-navy' : 'text-red-600' }} mt-2">R {{ number_format($netRevenue, 2) }}</p>
      <p class="text-xs text-kc-charcoal/70 mt-1">Collected − ECL − Bank charges</p>
    </div>
  </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
    ...
    <div class="kc-card">
      ...
      <p class="text-xs text-kc-charcoal/70 mb-3">Recognised (GL), matches the Income Statement for this period.</p>
      <table class="kc-table">
        <tbody>
          ...
          <tr><td>VAT on Fees</td><td class="text-right text-kc-charcoal/70">R {{ number_format($vatOnFees, 2) }}</td></tr>
  ...
  <div class="kc-card mb-6">
    ...
    <div class="grid grid-cols-2 gap-4">
      <div><p class="text-xs text-kc-charcoal/70">Deferred Interest</p><p class="font-display text-lg font-semibold">R {{ number_format($totalDeferredInterest, 2) }}</p></div>
      <div><p class="text-xs text-kc-charcoal/70">Deferred Fees</p><p class="font-display text-lg font-semibold">R {{ number_format($totalDeferredFees, 2) }}</p></div>
    </div>
    <p class="text-xs text-kc-charcoal/70 mt-2">Carried on balance sheet — recognised as instalments are collected on multi-month loans.</p>
  </div>
  @endif
  ...
  <div class="kc-card">
    ...
    <table class="kc-table">
      ...
      <tbody>
        ...
        @empty
        <tr><td colspan="5" class="text-center text-kc-charcoal/70 py-6">No payment data in this period.</td></tr>
        @endforelse
      </tbody>
    </table>
  </div>
