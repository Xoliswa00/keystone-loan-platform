  ...
  <div class="kc-card">
    <table class="kc-table">
      ...
      <tbody>
        @foreach($logs as $log)
        <tr>
          <td data-label="When" class="text-xs text-kc-charcoal/70">{{ $log->created_at->format('d M Y H:i') }}</td>
          ...
          <td data-label="IP" class="text-xs text-kc-charcoal/70">{{ $log->ip_address }}</td>
          <td data-label="Changes" class="text-xs max-w-xs truncate text-kc-charcoal/70">
            @if($log->new_values)
