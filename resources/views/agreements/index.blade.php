<x-app-layout>
  ...
  <div class="kc-card">
    ...
    <p class="text-center text-kc-charcoal/70 py-8">No NCA documents generated yet.</p>
    @else
    <div class="space-y-3">
      ...
      <div class="flex items-center justify-between p-4 rounded-xl border border-kc-silver-light hover:border-kc-gold/30 transition">
        <div>
          ...
          <p class="text-xs text-kc-charcoal/70 font-mono">{{ $ag->reference }}</p>
          <p class="text-xs text-kc-charcoal/70 mt-1">Generated {{ $ag->generated_at?->format('d M Y H:i') }}</p>
          @if($ag->accepted_at)
